...
void FrameCompressor::Compress(Gop& my_gop, int fnum){
	ChromaFormat cf=encparams.sparams.cformat;	
	Frame& my_frame=my_gop.GetFrame(fnum);
	FrameSort fsort=my_frame.GetFparams().fsort;
	int num_mv_bits;//number of bits written, without byte alignment
	...
	if (fsort!=I_frame){
		...
		my_motEst.DoME(my_gop,fnum,*mv_data);
		//next code mv data
		...
		GolombCode((encparams.BIT_OUT)->header,num_mv_bits);
		encparams.BIT_OUT->WriteToFile();

		int mv_bits=encparams.BIT_OUT->GetUnitBytes()*8;			//actual number of mv bits written including alignment
		int mv_head_bits=encparams.BIT_OUT->GetUnitHeadBytes()*8;	//and header
	...
	my_compcoder.Compress(my_gop.GetComponent(fnum,Y));
	if (cf!=Yonly){
		my_compcoder.Compress(my_gop.GetComponent(fnum,U));
		my_compcoder.Compress(my_gop.GetComponent(fnum,V));
	}
