...
void MotionEstimator::DoHierarchicalSearch(const FrameBuffer& my_buffer, int frame_num, MvData& mv_data){
	...
	depth = ( int) std::min( log(((double) pic_data.LengthX())/12.0)/log(2.0) , 
	...
	for (int i=1 ; i<=depth;++i)
		...
		scale_factor*=2;
		pic_down[i] = new PicArray(pic_data.LengthY()/scale_factor , pic_data.LengthX()/scale_factor);
		ref1_down[i] = new PicArray(ref1_data.LengthY()/scale_factor , ref1_data.LengthX()/scale_factor);

		if (ref1 != ref2)
			ref2_down[i] = new PicArray(ref2_data.LengthY()/scale_factor , ref2_data.LengthX()/scale_factor);
		xnumblocks = pic_down[i]->LengthX()/bparams.Xbsep();
		ynumblocks = pic_down[i]->LengthY()/bparams.Ybsep();

		if (( pic_down[i]->LengthX() )%bparams.Xbsep() != 0)
			xnumblocks++;

		if (( pic_down[i]->LengthY() )%bparams.Ybsep() != 0)
			ynumblocks++;
...
int ref_id,int level){	
	...
	int xnum = mv_array->LengthX();
	int ynum = mv_array->LengthY();
...
void MotionEstimator::SetChromaDC(const PicArray& pic_data, MvData& mv_data,CompSort csort)
{
	...
	for (int ymb_loc=0;ymb_loc<mv_data.mb.LengthY();++ymb_loc)
    {
		for (int xmb_loc=0;xmb_loc<mv_data.mb.LengthX();++xmb_loc)
        {
			...
			if (xmb_loc==mv_data.mb.LastX())
            {
				xbr = std::min(mv_data.mv1.LengthX(),xbr);
				xsubMBbr = std::min(mv_data.mv1.LengthX()>>1,xsubMBbr);
			}
			if (ymb_loc==mv_data.mb.LastY())
            {
				ybr = std::min(mv_data.mv1.LengthY(),ybr);
				ysubMBbr = std::min(mv_data.mv1.LengthY()>>1,ysubMBbr);
			}
