...
void ModeDecider::DoModeDecn( EncQueue& my_buffer, int pic_num )
{
    ...
    if (m_psort.IsInter())
    {
        ...
        m_pic_data = &(my_buffer.GetPicture( pic_num ).DataForME(m_encparams.FieldCoding()));

        // Set up the hierarchy of motion vector data objects
        m_me_data_set[0] = new MEData( m_encparams.XNumSB() , m_encparams.YNumSB() ,
                                       m_encparams.XNumBlocks()/4 , m_encparams.YNumBlocks()/4, num_refs );
        m_me_data_set[1] = new MEData( m_encparams.XNumSB() , m_encparams.YNumSB() ,
        ...
        m_ref1_updata = &(my_buffer.GetPicture( ref1 ).UpDataForME(m_encparams.FieldCoding()));

        if (num_refs>1)
        {
            ref2 = refs[1];
            m_ref2_updata = &(my_buffer.GetPicture( ref2).UpDataForME(m_encparams.FieldCoding()));
        ...
        for (m_ysb_loc=0 ; m_ysb_loc<m_encparams.YNumSB() ; ++m_ysb_loc )
        {
            for (m_xsb_loc=0 ; m_xsb_loc<m_encparams.XNumSB(); ++m_xsb_loc )
            {
                DoSBDecn();
            }//m_xsb_loc
        }//m_ysb_loc
