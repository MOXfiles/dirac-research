...
void MvDataCodec::DoWorkCode(MvData& in_data)
{
    ...
    for (mb_yp = 0, mb_tlb_y = 0;  mb_yp < in_data.mb.LengthY();  ++mb_yp, mb_tlb_y += 4)
    {
        for (mb_xp = 0,mb_tlb_x = 0; mb_xp < in_data.mb.LengthX(); ++mb_xp,mb_tlb_x += 4)
        {
...
void MvDataCodec::DoWorkDecode(MvData& out_data, int num_bits)
{
    ...
    for (mb_yp = 0,mb_tlb_y = 0; mb_yp < out_data.mb.LengthY(); ++mb_yp,mb_tlb_y += 4)
    {
        for (mb_xp = 0,mb_tlb_x = 0; mb_xp < out_data.mb.LengthX(); ++mb_xp,mb_tlb_x += 4)
        {
             //start with split mode
