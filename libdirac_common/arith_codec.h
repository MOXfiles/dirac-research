...
public:
	...
	*/	
	...
	/*!
		...
		bits written. 
		/param	in_data	the input to be compressed. Non-const, since the compression may be lossy.
	 */
	...
	/*!
		...
		subclasses.
		/param	out_data the output into which the decompressed data is written.
...
protected:
    ...
    public:
	    ...
	    const Triple & GetTriple(bool symbol) const { return (symbol ? trip1 : trip0); }

        //! Given a number, return the corresponding symbol and triple.
        /*!
            Given a number, which should be in the range [0,m_weight) return the corresponding symbol.
		    The range [0,m_weight) is partitioned into portions [0,count0), [count0,m_weight) corresponding
		    to 0 and 1.
         */	
	    bool GetSymbol(int num, Triple & trip_val) const
        {
		    if (num < trip0.Stop())
            {
			    trip_val = trip0;
...
protected:
	...
	virtual void Update(const int& context_num, const bool& symbol)=0;	
...
void ArithCodec<T>::EncodeTriple(const Triple &c)
{
	...
	range=(calc_t)(m_high_code - m_low_code) + 1;

	//formulae given we know we're binary coding	
	if (!c.Start()) // c.Start()=0, so symbol is 0, so m_low_code unchanged 
		m_high_code = m_low_code + (code_t)(( range * c.Stop() ) / c.Weight() - 1 );
	else //symbol is 1, so m_high_code unchanged
		m_low_code += (code_t)(( range * c.Start() ) / c.Weight() );				

    // This loop turns out new bits until m_high_code and m_low_code are far enough apart to have stabilized.
	do
	{
...
{
	calc_t range;
	range = (long) ( m_high_code - m_low_code ) + 1;
	m_count=(code_t)((((long) ( m_code - m_low_code ) + 1 ) * m_context_list[context_num].Weight() - 1 ) / range );
}
