...
/*!
    This class is a wrapper around the SequenceDecompressor class. The Sequence
    Decompressor class needs a full frame of data to be available to 
    decompress a frame successfully.  So, the DiracParser class uses the
    InputStreamBuffer class to store data until a chunk is available to 
    be processed and then invokes the SequenceDecompressor functions to
    ...
    SequenceDecompressor object is created using an InputStreamBuffer object
    which is manipulated the DiracParser. This ensures that data is always
    available for processing by the  SequenceDecompressor object.
*/
class DiracParser
{
public:
    ...
    */
    ...
    /*! Set the skip flag to the value specified in skip. If skip is true,
        the parser will skip decoding the next frame until the this function
        is called again with skip set to false
    */
