...
bool FrameDecompressor::Decompress(FrameBuffer& my_buffer){
	...
	if (!(decparams.BIT_IN->End()) && !header_failure){//if we've not finished the data, can proceed

		if (!skipped){//if we're not skipped then we can decode the rest of the frame
