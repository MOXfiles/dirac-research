...
void FrameDecompressor::Decompress(FrameBuffer& my_buffer){

	ChromaFormat cf=decparams.cformat;
	FrameParams fparams(decparams.cformat,my_buffer.GetFParams().xl,my_buffer.GetFParams().yl);

	//Get the frame header (which includes the frame number)	
	ReadFrameHeader(fparams);

	if (!(decparams.BIT_IN->End()) && !skipped){//if we've not finished the data and 
												//we're not skipped then we can decode the rest		
		...
		if (fsort!=I_frame){//do all the MV stuff		
		...
		if (fsort!=I_frame){//motion compensate to add the data back in if we don't have an I frame
...
void FrameDecompressor::ReadFrameHeader(FrameParams& fparams){
