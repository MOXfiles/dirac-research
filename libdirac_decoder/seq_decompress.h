...
public:
	...
	Frame& DecompressNextFrame();	//decompress the next frame in the sequence if not all done, else return last frame. 
									//Returns reference to the next decoded frame available for display
									//so that if decoding is fast enough the compressed version could be watched real-time
...
private:
	void ReadStreamHeader();	//reads the sequence data from the bitstream. This contains all the block and GOP information. 
	...
	//state variables for DecompressNextFrame
